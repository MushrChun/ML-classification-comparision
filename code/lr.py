...
# construct the LR classifier
lr = LogisticRegression(multi_class='multinomial',solver='sag',penalty='l2',C=0.1,class_weight='balanced',n_jobs=-1)
...
for train, test in skf.split(X, y.ravel()):
    ...
    lr.fit(X_scaled[train], y[train].ravel())

    # predict label of each test data
    predicted = lr.predict(X_scaled[test])
    # get probability of target label
    predicted_prob = lr.predict_proba(X_scaled[test])
